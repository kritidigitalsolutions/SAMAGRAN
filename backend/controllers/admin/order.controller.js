...
export const createOrderByAdmin = async (req, res) => {
  try {
    ...
    const amountBreakup = getAmountBreakupFromItems(normalizedItems, deliveryFee);

    const order = await Order.create({
      user,
      items: normalizedItems,
      totalAmount: amountBreakup.totalAmount,
      addressType: normalizedAddress.addressType || null,
      amountBreakup: {
        itemTotal: amountBreakup.itemTotal,
        deliveryFee: amountBreakup.deliveryFee,
      },
      address: normalizedAddress,
      paymentMethod: normalizePaymentMethod(paymentMethod),
      paymentStatus: normalizePaymentStatus(paymentStatus),
      paymentGateway: paymentGateway ? String(paymentGateway).trim() : null,
      orderStatus: normalizeOrderStatus(orderStatus),
      razorpayOrderId: razorpayOrderId ? String(razorpayOrderId).trim() : null,
      razorpayPaymentId: razorpayPaymentId ? String(razorpayPaymentId).trim() : null,
      razorpaySignature: razorpaySignature ? String(razorpaySignature).trim() : null,
    });
