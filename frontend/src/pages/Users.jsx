...
export default function Users() {
  ...
  const handleDeleteSelectedUsers = async () => {
    ...
    try {
      await Promise.all(selectedUserIds.map((id) => API.delete(`admin/user/${id}`)));
      setUsers((current) => current.filter((user) => !selectedUserIds.includes(user._id)));
      setSelectedUserIds([]);
      setSuccess("Selected users deleted successfully.");
      setError("");
    } catch (err) {
      setError(err.response?.data?.message || "Unable to delete selected users.");
    }
  };

  const handleDeleteSelectedOrders = async () => {
    if (!selectedUserOrderIds.length) return;
    if (!window.confirm(`Are you sure you want to delete ${selectedUserOrderIds.length} selected orders?`)) return;

    try {
      await Promise.all(selectedUserOrderIds.map((id) => API.delete(`admin/order/${id}`)));
      setSelectedUserOrders((current) => current.filter((order) => !selectedUserOrderIds.includes(order._id)));
      setSelectedUserOrderIds([]);
  ...
  const handleDeleteSelectedCartItems = async () => {
    if (!selectedUserCartIds.length) return;
    if (!window.confirm(`Are you sure you want to delete ${selectedUserCartIds.length} selected cart items?`)) return;

    try {
      await Promise.all(selectedUserCartIds.map((id) => API.delete(`admin/cart/${id}`)));
      setSelectedUserCart((current) => current.filter((item) => !selectedUserCartIds.includes(item._id)));
      setSelectedUserCartIds([]);
      setSuccess("Selected cart items deleted successfully.");
      setError("");
    } catch (err) {
      setError(err.response?.data?.message || "Unable to delete selected cart items.");
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-[30px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-6 shadow-[var(--admin-shadow)]">
        ...
        {/* 🔥 MAIN SUPER VISIBLE DELETE BANNER FOR USERS 🔥 */}
              ...
              <table className="admin-table min-w-full text-sm">
                ...
                <tbody>
                  {pagedUsers.map((user, index) => (
                    <tr key={user._id} className="border-b border-[#f0e3d1] align-top last:border-none hover:bg-[#faf6f0] dark:border-white/10 dark:hover:bg-white/5">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-1">
                          <span
                            className={`w-fit rounded-full px-2 py-1 text-xs font-semibold ${user.isBlocked
      ...
      {ordersModalUser && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4">
          <div className="w-full max-w-5xl rounded-3xl border border-[#dbc7a8]/60 bg-[var(--admin-surface)] p-6 shadow-[0_24px_70px_rgba(59,13,20,0.24)] dark:border-white/10">
            ...
            {/* 🔥 ORDERS BULK DELETE BANNER 🔥 */}
            {selectedUserOrderIds.length > 0 && (
              <div className="mb-4 flex items-center justify-between rounded-2xl border border-red-200 bg-red-50 p-4 shadow-sm dark:border-red-900/40 dark:bg-red-900/10">
                ...
                <button
                  ...
                  disabled={selectedUserOrderIds.length === 0}
                  className="flex items-center gap-2 rounded-xl bg-red-600 px-6 py-2.5 text-sm font-bold text-white shadow-md transition-all hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-gray-400 disabled:opacity-70 dark:disabled:bg-gray-700"
                >
                  <FiTrash2 /> Delete Selected
                </button>
                  ...
                  <table className="admin-table min-w-full text-sm">
                    ...
                    <tbody>
                      {pagedUserOrders.map((order, index) => (
                        <tr key={order._id} className="border-b border-[#f0e3d1] align-top last:border-none hover:bg-[#faf6f0] dark:border-white/10 dark:hover:bg-white/5">
                          <td className="px-4 py-3 text-center">
                            <input
                              type="checkbox"
      ...
      {cartModalUser && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4">
          <div className="w-full max-w-4xl rounded-3xl border border-[#dbc7a8]/60 bg-[var(--admin-surface)] p-6 shadow-[0_24px_70px_rgba(59,13,20,0.24)] dark:border-white/10">
            ...
            {/* 🔥 CART BULK DELETE BANNER 🔥 */}
            {selectedUserCartIds.length > 0 && (
              <div className="mb-4 flex items-center justify-between rounded-2xl border border-red-200 bg-red-50 p-4 shadow-sm dark:border-red-900/40 dark:bg-red-900/10">
                <div className="flex items-center gap-3">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-red-200 font-bold text-red-700 dark:bg-red-800 dark:text-red-200">
                    {selectedUserCartIds.length}
                  </div>
                  <span className="text-sm font-semibold text-red-800 dark:text-red-200">
                    Cart Items Selected
                  </span>
                ...
                <button
                  ...
                  disabled={selectedUserCartIds.length === 0}
                  className="flex items-center gap-2 rounded-xl bg-red-600 px-6 py-2.5 text-sm font-bold text-white shadow-md transition-all hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-gray-400 disabled:opacity-70 dark:disabled:bg-gray-700"
                >
                  <FiTrash2 /> Delete Selected
                </button>
                  ...
                  <table className="admin-table min-w-full text-sm">
                    ...
                    <tbody>
                      {pagedUserCart.map((item, index) => (
                        <tr key={item._id} className="border-b border-[#f0e3d1] align-top last:border-none hover:bg-[#faf6f0] dark:border-white/10 dark:hover:bg-white/5">
                          <td className="px-4 py-3 text-center">
                            <input
                              type="checkbox"
