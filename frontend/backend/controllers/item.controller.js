...
// ✅ Get all items (User)
export const getItemsUser = async (req, res) => {
  try {
  ...
  return {
    ...
    // first image for thumbnail/card
    thumbnail: item.media?.Images?.[0]?.replace(/\\/g, "/") || null,

    // all images of that product
    images:
...
// ✅ Get single item (User)
export const getSingleItemUser = async (req, res) => {
  try {
